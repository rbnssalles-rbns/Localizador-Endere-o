...
# Função para geocodificação via Google Maps
...
API_KEY = "AIzaSyDPt8ydgENhym9KYRqCN-tgpJjr9L6LWi0"

cd_lat, cd_lon = None, None
lat, lon = geocode_google(cd_endereco, API_KEY)
if lat and lon:
...
@st.cache_data
def geocode_dataframe_google(df, endereco_col="Endereco", api_key=API_KEY):
    results = []
    for _, row in df.iterrows():
        ...
        lat, lon = geocode_google(addr, api_key)
...
if arquivo:
    ...
    with st.spinner("Geocodificando endereços com Google Maps..."):
        coords = geocode_dataframe_google(df, endereco_col="Endereco", api_key=API_KEY)
